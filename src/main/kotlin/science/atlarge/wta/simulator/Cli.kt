...
fun parseCliArgs(args: Array<String>): CliValues {
    ...
    try {
        ...
        if (cores.isEmpty())
            throw ParseException("Need to supply core sizes")
        if (cores.minOrNull()!! <= 0)
            throw ParseException("A non-negative number of cores is required (was: ${cores.minOrNull()})")
        if(baseClocks == null)
        ...
        if (baseClocks.minOrNull()!! <= 0)
            throw ParseException("A non-negative number of base clock is required (was: ${cores.minOrNull()})")
